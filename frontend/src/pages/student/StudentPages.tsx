...
export const StudentCareerRecommendationsPage = () => {
  ...
  const aiAdvice = useAppQuery({
    queryKey: ['ai-guidance-career-advice', profile.data?.id, profile.data?.profileCompleteness],
    ...
    queryFn: async () => {
      ...
      if (!qualificationLevel || !interests || !skills || !location) {
        throw new Error('Please complete your profile (qualification level, interests, skills, and location) before generating AI guidance.');
      }
  ...
  if (aiAdvice.isError) return <ErrorState message={'Unable to generate AI guidance right now. Please try again shortly.'} />;

  const careers = aiAdvice.data?.recommendedCareers ?? [];

  return <Section title="AI Guidance">
    <p className="text-sm text-slate-500">Mode: live backend Gemini</p>

    <div className="space-y-2">
      <h3 className="font-semibold">Recommended careers</h3>
      {careers.length === 0 ? <p className="text-sm text-slate-500">No career recommendations yet.</p> : <div className="grid gap-3 md:grid-cols-2">{careers.slice(0, 10).map((career) => <article key={career.name} className="rounded border p-3 space-y-2 bg-white">
        ...
        <div>
          <p className="text-xs uppercase tracking-wide text-slate-500">Improvements</p>
          {career.improvements.length === 0 ? <p className="text-sm text-slate-500">No improvements suggested.</p> : <ul className="list-disc ml-5 text-sm">{career.improvements.map((improvement) => <li key={`${career.name}-${improvement}`}>{improvement}</li>)}</ul>}
        </div>
      </article>)}</div>}
    </div>
  </Section>;
