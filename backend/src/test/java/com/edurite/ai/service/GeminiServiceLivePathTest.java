...
class GeminiServiceLivePathTest {
    ...
    void sourcePipelineEmptyStillAttemptsGeminiAndReturnsLiveResponse() throws IOException {
        ...
        HttpServer server = createGeminiMockServer(
                "gemini-2.0-flash",
                """
                {
                  "candidates": [{
        ...
        try {
            GeminiService service = createService("gemini-2.0-flash", server);
            ...
            assertThat(response.warningMessage())
                    .contains("No external university sources were available");
        } finally {
            server.stop(0);
        }
    }
    ...
    void sourcePipelineEmptyAndGeminiFailureFallsBackAfterAttempt() {
        ...
        assertThat(response.warningMessage())
                .contains("Live AI guidance is temporarily unavailable");
    }
    ...
    void sourcePipelineWithSourcesStillUsesGeminiAsPrimary() throws IOException {
        ...
        try {
            GeminiService service = createService("gemini-2.5-flash", server);
    ...
    private static GeminiService createService(String model, HttpServer server) {
        GeminiService service = new GeminiService(new ObjectMapper(), new MockEnvironment());
        ReflectionTestUtils.setField(service, "configuredApiKey", "test-key");
    ...
    private static HttpServer createGeminiMockServer(
            String model,
            String responseBody,
